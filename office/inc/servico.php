<div class="container-fluid">
    ...
    <div class="row">
        ...
        <div class="col-md-2">
            ...
            <select name="queryPrioridade" class="form-control" id="Prioridade">
                <option value="nenhuma">Nenhuma</option>
                <option value="leve">leve ( <small> até 15 dias</small>)</option>
                <option value="moderado">moderado ( <small> até 11 dias </small>)</option>
                <option value="normal">normal ( <small> até 8 dias</small> )</option>
                <option value="urgente">urgente ( <small> até 4 dias</small)  ></option>
                <option value="atrasado">atrasado< ( <small> até 1 dias</small> ) </option>
            </select>
        </div>
        <div class="col-md-2">
    ...
    if (!empty($search)) {
        ...
        echo "<p>Sua Busca retornou os seguintes resultados. <a href='./index.php?p=servico'>limpar busca</a></p>";
    }
    ...
    <table class="table table-responsive-md">
        ...
        <tbody>
            ...
            if (!empty($search)) {
                ...
                $queryPrioridade = $search['queryPrioridade'];

                if ($queryPrioridade !== "nenhuma") {
           

                    if ($queryPrioridade == "leve") {
                        $hoje = date('Y-m-d');
                        $fim = date('Y-m-d', strtotime('+15 days', strtotime($hoje)));
                        $inicio = date('Y-m-d', strtotime('-15 days', strtotime($fim)));
                    } elseif ($queryPrioridade == "moderado") {
                        $hoje = date('Y-m-d');
                        $fim =  date('Y-m-d', strtotime('+11 days', strtotime($hoje)));
                        $inicio = date('Y-m-d', strtotime('-11 days', strtotime($fim)));
                    } elseif ($queryPrioridade == "normal") {
                        $hoje = date('Y-m-d');
                        $fim = date('Y-m-d', strtotime('+ 8 days', strtotime($hoje)));
                        $inicio = date('Y-m-d', strtotime('- 8 days', strtotime($fim)));
                    } elseif ($queryPrioridade == "urgente") {
                        $hoje = date('Y-m-d');
                        $fim = date('Y-m-d', strtotime('+4 days', strtotime($hoje)));
                        $inicio = date('Y-m-d', strtotime('-4 days', strtotime($fim)));
                    } elseif ($queryPrioridade == "atrasado") {
                        $hoje = date('Y-m-d');
                        $fim = date('Y-m-d', strtotime('+1 days', strtotime($hoje)));
                        $inicio = date('Y-m-d', strtotime('-1 days', strtotime($fim)));
                    }

                } else {

                    // Your existing else block remains unchanged
            ...
            foreach ($servicos->getResult() as $value) {
                ...
                $dados[] = [
                    ...
                    "obs" => $value['obs'],
                    "servico" => $value['name'],
                    "valor" => number_format($value["price"] / 100, 2, ",", "."),
                    "forma_pgto" => $value['method_payment'],
                    "parceiro" => $value['name'],
                ];
                ...
                <tr>
                    <?php if ($_SESSION["nivel"] == 3) { ?>
                        ...
                        <td><?php
                            ...
                            // Obtém a diferença em dias (incluindo dias negativos se ultrapassou a data)
                            $diferencaEmDias = $diferenca->format('%r%a');

                            $prazo = "Prazo: " . $diferencaEmDias . " dias </br>";
    ...
    ?>
        ...
        <form action="./relatorio_servicos.php" method="post" target="_blank">
            <input type="hidden" name="dados" value="<?php echo htmlentities(json_encode($dados)); ?>">
        ...
        <!-- <p><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-download" viewBox="0 0 16 16">
            ...
            </svg> <a href="./relatorio_servicos.php?array=<?php print_r($dados)  ?>" target="_blank">Download Relatório</a> -->
